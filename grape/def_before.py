    ...
    for i in range(data_num):
        ...
        for j in range(image_num):
            ...
            for k in range(len(before)):
                search_image_name = "/Users/homura/Desktop/grape_dateset/20190531/" + before[k] + "/" + image_name
            ...
            if same_file != 1:
                ...
                json_load[data_name][i]["image"][j] = "/Dropbox/grape/20190531/" + before[only_file] + "/" + image_name
    fi.close()
    return json_load
...
'''
def main_movie_before(json_load):
    fi = open("error_movie.txt",'w')
    data_name = json_load.keys()            #file_before / file_after
    data_name = "".join(data_name)          #list -> str
    data_num = len(json_load[data_name])
    for i in range(data_num):
        ...
        if movie_name != "":
            if variety == "Kyoho":
                search_movie = glob.glob("/Users/homura/Desktop/grape_dateset/20190531/YN10101282/" + movie_name)
                if len(search_movie) == 1:
                    json_load[data_name][i]["movies"] = "/Dropbox/grape/20190531/YN10101282/" + movie_name
                else:
                    fi.write(str([json_load[data_name][i]["grapeID"],movie_name]))
                    fi.write("\n")
            elif variety == "Sunverde":
                search_movie = glob.glob("/Users/homura/Desktop/grape_dateset/20190531/YN10101281/" + movie_name)
                if len(search_movie) == 1:
                ...
                search_movie = glob.glob("/Users/homura/Desktop/grape_dateset/20190531/YN10101321/" + movie_name)
                if len(search_movie) == 1:
                    json_load[data_name][i]["movies"] = "/Dropbox/grape/20190531/YN10101321/" + movie_name
                else:
                    fi.write(str([json_load[data_name][i]["grapeID"],movie_name]))
                    fi.write("\n")
