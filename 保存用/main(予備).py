...
for i in range(len(color_feature_train[0])):
    ...
    for j in range(detected_num):
        feature += color_feature_train[j][i] / detected_num
    color_feature_med.append(feature)
for i in range(len(HoG_feature_train[0])):
    feature = 0
    for j in range(detected_num):
        feature += HoG_feature_train[j][i][0] / detected_num
    HoG_feature_med.append(feature)
for i in range(len(gist_feature_train[0])):
    feature = 0
    for j in range(detected_num):
        feature += gist_feature_train[j][i] / detected_num
    gist_feature_med.append(feature)
...
if candidate_num < 2:
    print("error:detected candidate_num = 0")
    sys.exit()
x_center = []
y_center = []
detected_num = 0
